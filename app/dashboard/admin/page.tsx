...
export function CameraFeed({ onTranslation, recentTranslations }: CameraFeedProps) {
  ...
  const startMP = useCallback(async () => {
    ...
    const isRearMobile = facingModeRef.current === "environment" && isMobile
    const camWidth  = isRearMobile ? 720  : (isMobile ? 480  : 1280)
    const camHeight = isRearMobile ? 1280 : (isMobile ? 640  : 720)
      ...
      <div className="cf-root flex flex-col gap-4 w-full">
        ...
        <div className="cf-card" style={{ padding: 10, position:'relative' }}>
          <div
            ...
            style={{ aspectRatio: facingMode === "environment" && isMobile ? "9/16" : (isMobile ? "3/4" : "16/9") }}
          >
            ...
            {/* Loading overlay */}
            {cameraOn && isLoading && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3" style={{ background: 'rgba(219,234,254,0.85)', borderRadius: 22 }}>
                <div style={{ width: 40, height: 40, border: '4px solid #bfdbfe', borderTop: '4px solid #2563eb', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                <p style={{ fontSize: 13, color: '#1d4ed8', fontWeight: 600 }}>กำลังเปิดกล้อง...</p>
              </div>
            )}
